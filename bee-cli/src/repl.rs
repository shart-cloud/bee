...
use crate::session::{self, EX_IOERR, EX_SOFTWARE, EX_USAGE};
...
async fn repl(args: ReplArgs) -> ExitCode {
    ...
    let resolved_policy = session::resolve_grants(
        ...
        &skills,
        &mut registry,
        &(prompt_consent as fn(&bee_harness::skills::GrantRequest<'_>) -> bool),
        CMD,
    )
    .await;
    ...
    let session_out: Option<bee_harness::repl::ReplSession> = {
        #[cfg(feature = "tui")]
        {
            if use_tui {
                if let Err(e) =
                    bee_harness::tui::run(session.model.as_ref(), &mut registry, &mut sbox, &config)
                        .await
                {
...
fn prompt_consent(request: &bee_harness::skills::GrantRequest<'_>) -> bool {
    use std::io::Write;
