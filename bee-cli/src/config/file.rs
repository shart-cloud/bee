...
pub use bee_harness::viz::theme::ThemeConfig;
...
///
/// The user path comes from `bee_harness::viz::theme::config_path` — the same function that has
...
pub fn discover(cwd: &Path, explicit: Option<&Path>) -> Result<Vec<Layer>, LoadError> {
    ...
    if let Some(user) = bee_harness::viz::theme::config_path() {
