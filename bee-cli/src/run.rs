...
use bee_harness::{run_batch, run_episode, BatchConfig, EpisodeTranscript, Scenario};

use crate::config::{self, EffectiveConfig, Flags};
use crate::session::{self, EX_DATAERR, EX_IOERR, EX_SOFTWARE, EX_UNAVAILABLE, EX_USAGE};
...
fn progress_sink(quiet: bool) -> Option<bee_harness::ProgressSink> {
    if quiet {
        None
    } else {
...
fn install_presentation_for_batch(args: &RunArgs, cwd: &Path) {
    if let Err(e) = bee_harness::set_non_dumpable() {
...
async fn run_concurrent_mode(
    ...
    use bee_harness::ProviderConfig;
