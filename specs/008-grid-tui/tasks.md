...
- [X] T018 [US1] Event loop in `bee-harness/src/tui/mod.rs`: `crossterm` `EventStream` + `tokio::select!` over terminal events, the `SessionEvent` channel, resize, and an on-demand tick; redraw once per handled batch (depends on T013, T016, T017; research D3). `run()` owns the shared conversation core (like `run_repl`, minus the readline editor) and drives `run_exchange` through a `SessionSink` per submitted line; `run_turn` pumps streaming output + input while the exchange future is in flight, so Ctrl-C quits mid-turn. The tick is armed only during a turn ("on-demand").
- [X] T019 [US1] Add `--tui`/`--no-tui` flags and launch the full-screen front-end in `bee-harness/src/bin/bee-repl.rs` (minimal opt-in; capability-based auto-selection + graceful fallback land in US3/T033) (depends on T018). `--tui` conflicts with `--no-tui`; without the `tui` build feature `--tui` warns and runs inline. TUI path yields no transcript yet, so `--save` is a no-op there.
