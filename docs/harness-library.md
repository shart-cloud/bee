...
The commands that drive all of this — `bee run`, `bee repl`, `bee metrics` — live in `src/app/`,
reached only from `src/main.rs`. Library and application are one package because the library has
exactly one consumer (ADR-0002).
