...
use bee_harness::render_api::{register, RenderContext, RenderOutcome};
use rhai::Engine;
...
fn panel_fade_in_draws_a_panel_and_requests_no_effect() {
    ...
    assert!(
        matches!(
            &out.panel_ops[0],
            bee_harness::render_spec::PanelOp::Upsert { effect: None, .. }
        ),
...
fn slide_dashboard_attaches_a_distinct_effect_to_each_panel() {
    use bee_harness::render_spec::{EffectSpec, PanelOp};
